...
范围与证据
  → 规划 index
  → index 校验
  → Course Studio G2 结构化知识点审核（适用时，跨 turn）
  → points 任务
  → points 全局复核
  → index 同步
  → 动画请求归并
...
上一个阶段未通过时，不启动下一个阶段。不要让子智能体在缺少全量 ID 注册表时猜测前置关系。Course Studio 工作流必须等后端审批回执与当前 `id/title/顺序` 哈希一致后再创建 points 任务；request marker、普通聊天或通用 `question` 都不能打开此门。
...
3. 仍失败：结束原任务后，把该单点文件所有权转交给新的详情 worker；主智能体不得直接接管 points 或动画请求文件，也不得让两个任务同时拥有它们。
4. 发现索引本身错误：暂停受影响任务，由主智能体串行修订索引和 manifest；Course Studio 中这会使 G2 回执失效，必须重新审核后再生成任务清单。
