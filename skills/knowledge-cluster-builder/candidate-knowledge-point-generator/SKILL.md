...
|---|---|
| index 校验 | `validate-index` |
| points 校验 | `validate-points` |
| 同步 index | `sync-index` |
| 构建动画注册 | `build-animation-registry` |
| 全量内容校验 | `validate-all` |

Skill 自测没有课程会话 action，只能由 Course Studio 部署测试执行；Agent 不得为此请求 Bash 权限。
...
9. 在 Course Studio 创建流水线中，index 校验通过后必须先取得 G2 结构化知识点审核回执；审核前不得创建任何详情或动画请求文件，也不得启动详情子智能体。
...
完成后运行（仅独立调试；Course Studio 使用 `validate-index`）：
...
在 Course Studio 创建流水线中，index 校验通过仍不等于身份已冻结：必须返回 `knowledge-pipeline-orchestrator` 调用 `course_pipeline` 的 `review-knowledge-points` 动作。工具返回 `pending` 时结束当前 turn；普通聊天确认、`question` 回答或模型可写的 request 文件都不是批准。只有下一 turn 重跑后返回 `approved`，且 `init-points` 动作验证同一份 `id/title/顺序` 哈希成功，才可进入下节并调度详情任务。
...
Course Studio 中这些文件始终由 `course-content-worker` 拥有；主智能体不得直接写入或在重试时接管。
...
运行（仅独立调试；Course Studio 依次使用 `validate-points`、`sync-index`）：
...
组件完成后串行运行（仅独立调试；Course Studio 使用 `build-animation-registry`）：
...
运行（仅独立调试；Course Studio 使用 `validate-all`，不运行 Skill 自测）：
