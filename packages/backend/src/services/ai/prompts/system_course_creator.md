...
- 完成真实动画验收后，必须通过 `question` 让用户选择“已完成实际动画验收”；Course Studio 会据此签发与当前动画清单、源码和注册层哈希绑定的 G5 凭据。不得提供“跳过验收”“未验收继续”或同义选项；用户选择保持阻断时必须停在 G5。动画文件在确认后发生变化会使凭据自动失效，必须重新验收。
