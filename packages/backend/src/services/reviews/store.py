...
def is_strict_g2_successor(
    ...
    return (
        ...
        and pending_review.get("kind") == "prerequisites"
        and pending_review.get("gate") == "G6_PREREQUISITE_REVIEW"
...
def _review_audit_sha256(graph: dict[str, Any]) -> str:
    ...
    return _hash_json(
        _normalized_review_audit(generation.get("refinedPrerequisiteEdges"))
    )
...
class CourseReviewStore:
        ...
        expected_paths = (
            {
                ...
                self._approval_path(session_root, course_id, "prerequisites"),
            ...
            if kind == "knowledge-points"
                ...
                paths["graph"],
                self._approval_path(session_root, course_id, "prerequisites"),
                self._resource_path(session_root, course_id, "prerequisites"),
            }
        )
        expected_relatives = {
    ...
    def _approval_matches(
        ...
        *,
        ...
        if kind != "prerequisites":
            return True
        return bool(
            _SHA256.fullmatch(str(approval.get("prerequisites_sha256") or ""))
    ...
    def _prerequisite_snapshot_from_source(
        self, source: dict[str, Any], course_id: str
        ...
        public_points = [{
            ...
            "difficulty": str(point.get("difficulty") or ""),
            "importance": point.get("importance"),
            "cluster_ids": deepcopy(point.get("clusterIds") or []),
        ...
        for point in points:
            ...
            for related_id_value in related:
                if not isinstance(related_id_value, str):
                    continue
                related_id = related_id_value.strip()
                if related_id not in point_ids or related_id == point_id:
                    continue
    ...
    def _prerequisite_snapshot(
        self, session_root: Path, course_id: str
    ) -> tuple[dict[str, Any], str, str, str, str]:
        source, _ = self._prerequisite_source(session_root, course_id)
        return self._prerequisite_snapshot_from_source(source, course_id)
        ...
        *,
        ...
        if kind == "knowledge-points":
            ...
            (
                ...
                prerequisites_sha256,
                review_audit_sha256,
            ) = self._prerequisite_snapshot(session_root, course_id)
    ...
    def pointer(resource: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            "review_url": (
                ...
                + ("points" if resource["kind"] == "knowledge-points" else "prerequisites")
            )
        }
    ...
    def _apply_prerequisite_operations(
        self,
        session_root: Path,
        resource: dict[str, Any],
        operations: list[dict[str, Any]],
        ...
        for operation in operations:
            if not isinstance(operation, dict) or set(operation) != {
                ...
                raise CourseReviewValidationError(
                    "依赖审核操作只允许 op/dependent_id/prerequisite_id/reason"
                )
            op = operation.get("op")
            if op not in {"add", "remove"}:
                raise CourseReviewValidationError("依赖审核只支持 add 或 remove")
            ...
            if op == "add":
                ...
                if frozenset(edge) in related_pairs:
                    raise CourseReviewValidationError(
                        "该知识点对已有 related 关系；第一版审核只修改 prerequisites，不能自动删除 related"
                    )
                next_edges.add(edge)
            else:
            ...
            touched.add(edge)
            operation_reasons[(op, dependent_id, prerequisite_id)] = reason
        ...
        approval_path = self._approval_path(session_root, course_id, "prerequisites")
        ...
        *,
        ...
        with self._lock, self._review_transaction_lock():
            ...
            prerequisite_approval = self._approval_path(
                session_root, course_id, "prerequisites"
            )
            resource_path = self._resource_path(session_root, course_id, kind)
            if kind == "knowledge-points":
                ...
                source, source_bytes = self._prerequisite_source(session_root, course_id)
                (
                    _,
                    ...
                    _,
                    _,
                ) = self._prerequisite_snapshot_from_source(source, course_id)
            ...
            try:
                if kind == "knowledge-points":
                    ...
                    expected_outputs = self._apply_prerequisite_operations(
                        session_root,
                        resource,
                        operations,
                        source,
    ...
    def _resume_content(
        ...
        if resource["kind"] == "knowledge-points":
            ...
            display_content = (
                f"已确认 {resource['course_title']} 的 prerequisites"
                + (f"，并提交 {operation_count} 项变更" if operation_count else "")
            )
            resume_message = (
                f"依赖关系审核 {review_id} 已由用户在结构化审核页完成，"
                "后端已验证 DAG、机械写回并写入有效回执。请依次调用 course_pipeline 的 "
                ...
                f'{{"action":"review-prerequisites","courseId":"{course_id}"}}；'
